...
def adjust_unet_input_channels(unet, channels):
    with torch.no_grad():
        new_conv_in = torch.nn.Conv2d(channels, unet.conv_in.out_channels, unet.conv_in.kernel_size, unet.conv_in.stride, unet.conv_in.padding)
        new_conv_in.weight.zero_()
        new_conv_in.weight[:, :4, :, :].copy_(unet.conv_in.weight)
        new_conv_in.bias = unet.conv_in.bias
        unet.conv_in = new_conv_in

# Change UNet forward function
...
# Load model weights based on the presence of a background image
def load_model_weights(background_present):
    if background_present:
        model_path = './models/iclight_sd15_fbc.safetensors'
        adjust_unet_input_channels(unet, 12)
    else:
        model_path = './models/iclight_sd15_fc.safetensors'
        adjust_unet_input_channels(unet, 8)
...
# Unified process_relight function
@torch.inference_mode()
